                    ...
                    <div class="row g-4">
                        <div class="col-md-12 text-center mb-3">
                            <div class="position-relative d-inline-block">
                                <img id="avatarPreview" src="{{ $device->avatar_url }}" class="rounded-circle shadow-sm" width="100" height="100" style="object-fit: cover; background-color: #fff;">
                            ...
                            <div class="small text-muted mt-2">Optional: Update Device Image</div>
                            ...
                            <div class="form-floating">
                                <select class="form-select select2 @error('user_id') is-invalid @enderror" id="user_id" name="user_id">
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ old('user_id', $device->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->email }})</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-12 mt-5 d-flex justify-content-end gap-2">
                            ...
                            <button type="submit" class="btn btn-primary px-4">Update Device</button>
                        </div>
                    </div>
                </form>
            </div>
